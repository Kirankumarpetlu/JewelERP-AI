...
const chartRangeStart = new Date(2025, 0, 1);

function buildMonthTimeline() {
  const today = new Date();
  const months: Array<{ key: string; label: string; shortLabel: string; month: string; year: number }> = [];
  const cursor = new Date(chartRangeStart);
...
const timelineMonths = buildMonthTimeline();

function buildEstimatedSeries(currentRate: number, shape: number[]) {
  const anchor = shape[shape.length - 1] || 1;

  return timelineMonths.map((point, index) => {
    const ratio = (shape[index] || anchor) / anchor;
    return {
      month: point.shortLabel,
      label: point.label,
      dateKey: point.key,
      price: Number((currentRate * ratio).toFixed(2)),
    };
  });
}

function normalizeYearSeries(
  points: Array<{ month: string; price?: number | null; date?: string }> | undefined,
  currentRate: number,
  estimatedShape: number[],
) {
  const validPoints = (points || []).filter((point): point is { month: string; price: number; date?: string } => typeof point.price === "number");

  if (validPoints.length < 2) {
    return buildEstimatedSeries(currentRate, estimatedShape);
  }

  const byMonth = new Map(
    validPoints.map(point => {
      const dateKey = point.date ? point.date.slice(0, 7) : "";
      return [dateKey, point.price] as const;
    }),
  );
  let previousPrice: number | null = null;

  return timelineMonths.map(point => {
    const livePrice = byMonth.get(point.key);
    if (typeof livePrice === "number") {
      previousPrice = livePrice;
      return { month: point.shortLabel, label: point.label, dateKey: point.key, price: livePrice };
    }

    return { month: point.shortLabel, label: point.label, dateKey: point.key, price: previousPrice };
  });
}
...
export default function Dashboard() {
  ...
  const goldPriceData = normalizeYearSeries(
    data?.gold_price_trend,
    goldRate,
    [0.74, 0.76, 0.79, 0.82, 0.84, 0.86, 0.88, 0.9, 0.92, 0.95, 0.98, 1.0, 1.03, 1.01, 1.05],
  );

  const silverPriceData = normalizeYearSeries(
    data?.silver_price_trend,
    silverRate,
    [0.68, 0.7, 0.73, 0.75, 0.77, 0.8, 0.83, 0.86, 0.88, 0.91, 0.95, 0.98, 1.0, 0.97, 1.02],
  );
  ...
  return (
    <div className="space-y-8">
      <AnimatedSection>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Welcome back to ERP.ai</p>
          </div>
          {loading && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard title="Gold Rate" value={`${fmt(goldRate)}/g`} icon={CircleDot} trend="+2.9%" trendUp highlight="gold" delay={0.15} />
        <StatCard title="Silver Rate" value={`${fmt(silverRate)}/g`} icon={CircleDot} trend="+3.4%" trendUp highlight="silver" delay={0.2} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <AnimatedSection delay={0.1}>
          <div className="glass-card p-5">
            <h3 className="font-display text-lg font-semibold mb-4">Gold Price Trend (2025-2026)</h3>
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={goldPriceData}>
                <defs>
                  <linearGradient id="goldGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#F8FAFC" stopOpacity={0.32} />
                    <stop offset="95%" stopColor="#F8FAFC" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(220 10% 22%)" />
                <XAxis dataKey="month" stroke="hsl(220 8% 68%)" fontSize={11} interval={1} />
                <YAxis stroke="hsl(220 8% 68%)" fontSize={12} />
                <Tooltip
                  {...chartTooltipStyle}
                  labelFormatter={(_, payload) => payload?.[0]?.payload?.month ?? ""}
                  formatter={(value: number) => [fmt(value), "Gold"]}
                />
                <Area type="monotone" dataKey="price" stroke="#F8FAFC" fill="url(#goldGrad)" strokeWidth={2} connectNulls />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </AnimatedSection>
        ...
        <AnimatedSection delay={0.15}>
          <div className="glass-card p-5">
            <h3 className="font-display text-lg font-semibold mb-4">Silver Price Trend (2025-2026)</h3>
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={silverPriceData}>
                <defs>
                  <linearGradient id="silverGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#CBD5E1" stopOpacity={0.34} />
                ...
                <Tooltip
                  {...chartTooltipStyle}
                  labelFormatter={(_, payload) => payload?.[0]?.payload?.month ?? ""}
                  formatter={(value: number) => [fmt(value), "Silver"]}
                />
                <Area type="monotone" dataKey="price" stroke="#CBD5E1" fill="url(#silverGrad)" strokeWidth={2} connectNulls />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </AnimatedSection>
          ...
          <div className="glass-card p-5">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={revenueTrend}>
                ...
                <YAxis stroke="hsl(220 8% 68%)" fontSize={12} tickFormatter={v => `\u20B9${(v / 1000).toFixed(0)}k`} />
                <Tooltip {...chartTooltipStyle} formatter={(v: number) => [fmt(v), "Revenue"]} />
                <Area type="monotone" dataKey="revenue" stroke="#CBD5E1" fill="url(#revGrad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </AnimatedSection>
          ...
          <div className="glass-card p-5">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={ordersTrend}>
                ...
                <Line type="monotone" dataKey="orders" stroke="#94A3B8" strokeWidth={2} dot={{ fill: "#E2E8F0", r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </AnimatedSection>
